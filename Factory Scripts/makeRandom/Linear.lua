...
-- Adopted from JWildfire's  LinearRandomFlameGenerator.java --------
-- Written by: Andreas Maschke
-- Shortname: Linear only
-- Variations Required: linear

local xformCount = math.random(2,6)
local scl = 1.
local tscl = 2.
local r0 = 0.;
local contRot = math.random() < 0.5

flame = makeBlankFractal(xformCount)

for i = 1, xformCount do	
	local xform = flame.xforms[i]
	
	if contRot then
        local r0 = r0 + 45.0 * math.random() - 9.0 * math.random()
		affine_Prot(xform, r0);
	elseif math.random() < 0.5 then
		affine_Prot(xform, 360. * math.random())
	else
		affine_Prot(xform, -360. * math.random())
	end
	affine_translate(xform,  (2.0 * math.random() - 1.0) * tscl, (2.0 * math.random() - 1.0) * tscl)

    scl  = scl  * (0.8 + math.random() * 0.1);
    tscl = tscl * (0.8 + math.random() * 0.1);
	affine_scale(xform, scl)

	xform.weight = scl * math.random() * 19.9 + 0.1
	xform.color = math.random()

    table.insert(xform.variations, { name="linear", weight=math.random() * 0.5 + 0.5 })
end

--[[ Original Jave version as written by Andreas Maschke
