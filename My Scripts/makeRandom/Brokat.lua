...
-- Adopted from JWildfire's  BrokatRandomFlameGenerator.java --------
-- Written by: Andreas Maschke
-- Shortname: Brokat
-- Variations Required: curl, juliascope, julian, bubble, pre_blur

local function randomVariation()
-- These are the names from Andreas' script - not all of these are available in Fractal Architect
...
local requiredVariations = { "curl", "juliascope", "julian", "bubble", "pre_blur", "julia3D", "julia3Dz" }
...
if not variationSet.hasVariations(requiredVariations) then
    if not variationSet.switchToFirstVariationSetWithRequiredVariations(requiredVariations) then
        local uuid = variationSet.makeSetWithVariations(requiredVariations, "Brokat")
...
local fncNames = { "juliascope", "julia3D", "julia3Dz", "julian" }
local variation = {}
...
table.insert(xform.variations, { name=randomVariation(), weight=0.01 + math.random() * 0.04 })
